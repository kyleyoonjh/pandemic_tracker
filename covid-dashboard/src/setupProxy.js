...
const waitUntilWhoFileActive = async ({ apiKey, fileName, requestTimeoutMs, httpsAgent }) => {
...
const ensureWhoFileUploaded = async ({ apiKey, requestTimeoutMs, httpsAgent }) => {
  ...
  if (!fileName) {
    throw new Error('Gemini upload did not return file name.');
  }
  const activeFile = await waitUntilWhoFileActive({ apiKey, fileName, requestTimeoutMs, httpsAgent });
  cachedWhoFileRef = {
        ...
        try {
          if (provider === 'groq' && groqApiKey) {
            ...
            if (answer) {
              ...
              res.json({ answer: finalAnswer, provider: 'groq-llama4-scout' });
              return;
            }
          ...
          if (provider === 'gemini' && geminiApiKey) {
            ...
            const whoFileRef = await ensureWhoFileUploaded({ apiKey: geminiApiKey, requestTimeoutMs, httpsAgent });
            const contents = [
              {
                role: 'user',
                parts: [
            ...
            if (answer) {
              ...
              res.json({ answer: finalAnswer, provider: 'gemini' });
              return;
            }
          ...
          if (provider === 'openai' && openAiApiKey) {
            ...
            if (answer) {
              ...
              res.json({ answer: finalAnswer, provider: 'openai-fallback' });
              return;
            }
